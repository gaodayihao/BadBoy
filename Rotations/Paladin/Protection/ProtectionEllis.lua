...
----------------
    ...
    -----------------------
            local function hasDefender()
                if debuff.eyeOfTyr[units.dyn5].exists
                    ...
                    or (buff.divineShield.exists and talent.knightTemplar)
                then
                    return true
                end
                return false
        ...
        -- Action List - Default
            ...
            -- Shield Of The Righteous
                ...
                -- shield_of_the_righteous,if=(talent.bastion_of_light.enabled&!talent.seraphim.enabled&cooldown.bastion_of_light.up)&!(debuff.eye_of_tyr.up&buff.aegis_of_light.up&buff.ardent_defender.up&buff.guardian_of_ancient_kings.up&buff.divine_shield.up&buff.potion.up)
                    if lastSpell ~= spell.shieldOfTheRighteous and ((not talent.seraphim or charges.shieldOfTheRighteous > 2) and not hasDefender()
                        or
                        talent.bastionOfLight and talent.seraphim and buff.seraphim.exists and cd.bastionOfLight == 0 and not hasDefender()
                        or
                        talent.bastionOfLight and not talent.seraphim and cd.bastionOfLight == 0 and not hasDefender())
                    then
    ...
    tinsert(cPalProtection.rotations, {
        name = rotationName,
