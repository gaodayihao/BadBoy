...
---------------
    local function createOptions()
        ...
        -- General Options
            ...
            -- Auto Target
                bb.ui:createCheckbox(section,"Auto Target")
            -- APL
                bb.ui:createDropdownWithout(section, "APL Mode", {"|cffFFFFFFSimC710-01","|cffFFFFFFAMR"}, 1, "|cffFFFFFFSet APL Mode to use.")
            -- Dummy DPS Test
        ...
        -- Cooldown Options
            ...
            -- Potion
                bb.ui:createDropdown(section, "Potion", {"|cffFFFF00Old War","|cffFF0000Prolonged Power"}, 1)
            -- Flask / Crystal
...
----------------
    ...
    --------------
            ...
            local autoTarget                                    = isChecked("Auto Target")
    ...
    --------------------
		-- Action List - Auto Target
			local function actionList_AutoTarget()
				if not inCombat or profileStop==true then return end
				if autoTarget == false then return end
				if isValidUnit("target") then return end
				local theEnemies = nil;
				if #enemies.yards5 > 0 then
					theEnemies = enemies.yards5
				elseif #enemies.yards8 > 0 then
					theEnemies = enemies.yards8
				else
					local enemies35 = getEnemies("player", 35)
					if #enemies35 == 0 then return end
					theEnemies = enemies35
				end
				...
				if targetUnit and UnitIsEnemy("player",targetUnit) then
        ...
        -- Action List - Cooldowns
            ...
            -- Potion
                    ...
                    if isChecked("Potion") and inRaid and feralSpiritRemain > 5 and getTimeToDie("target") <= 30 then
    ...
    --------------------------
        ...
        ---------------------------
                ...
                -- Boulderfist
                        ...
                        if buff.remain.boulderfist < gcd or (charges.frac.boulderfist > 1.75 and power <= 100 and ((mode.rotation == 1 and #enemies.yards5 <= 2) or mode.rotation == 2)) then
                            if cast.boulderfist() then return end
                        end
                -- Crash Lightning
